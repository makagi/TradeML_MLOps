...
@register_component('xgboost')
@dsl.component(
    base_image="python:3.11",
    packages_to_install=["pandas==2.2.3", "scikit-learn==1.5.2", "xgboost==2.0.3", "gcsfs==2024.10.0", "joblib==1.4.2", "google-cloud-aiplatform==1.70.0"]
)
def xgboost_component(
    ...
    max_depth: int = 6,  # XGBoost default is usually 6
    learning_rate: float = 0.1,
    random_state: int = 42,
    project_id: str = "",
    experiment_name: str = "default-experiment",
) -> dict:
    """XGBoostでモデルを学習するコンポーネント"""
    import pandas as pd
    ...
    try:
        ...
        if project_id:
            ...
            run_name = f"xgb-{features.replace(',', '-')[:50]}"
    ...
    print(f"Training XGBoost with {len(valid_features)} features, {len(X)} samples")
